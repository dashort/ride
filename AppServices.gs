 ...
 */
/**
 * Simple fix function that doesn't depend on missing diagnostic functions
 * Add this to your Code.gs or AppServices.gs file
 */

/**
 * BACKEND FIX FOR getPageDataForRiders FUNCTION
 * Add this to your AppServices.gs or Code.gs file to fix the "no data received from server" issue
 */

/**
 * STEP 1: Fixed getPageDataForRiders function
 * This function should return data in the format expected by the frontend
 */
function getPageDataForRiders(user) {
  console.log('🔄 === getPageDataForRiders CALLED ===');
  ...
  try {
    ...
    if (!authenticatedUser) {
      console.log('🔐 No user provided, attempting authentication...');
    ...
    // Step 2: Get riders data
    console.log('📊 Getting riders data...');
    let riders = [];
    
    try {
      // Try the existing getRiders function first
      if (typeof getRiders === 'function') {
        console.log('📋 Using getRiders() function...');
        riders = getRiders();
        console.log(`✅ getRiders() returned ${riders ? riders.length : 0} riders`);
      } else {
        console.log('⚠️ getRiders() function not found, using alternative method...');
        riders = getAlternativeRidersData();
      }
      
    } catch (ridersError) {
      console.log('❌ Error getting riders data:', ridersError.message);
      // If getRiders fails, try direct sheet access
      riders = getDirectRidersData();
    }
    
    // Step 3: Validate and filter riders
    if (!Array.isArray(riders)) {
      console.log('⚠️ Riders data is not an array, converting...');
      riders = riders ? [riders] : [];
    }
    
    console.log(`📊 Raw riders count: ${riders.length}`);
    
    // Filter for active riders (matching your existing logic)
    const activeRiders = riders.filter(rider => {
      ...
      const hasName = rider.name || rider['Full Name'] || rider.fullName;
      
      // Include riders that are active, available, or have no status (default active)
    ...
    console.log(`✅ Filtered to ${activeRiders.length} active riders with names`);
    ...
    console.log('📤 Returning response:', {
      ...
      user: response.user ? response.user.name : 'none',
      ridersCount: response.riders.length,
      statsTotal: response.stats.totalRiders
    });
...
/**
 * STEP 2: Alternative method to get riders data if getRiders() fails
 */
function getAlternativeRidersData() {
  console.log('🔄 Getting riders data using alternative method...');
  
  try {
    // Try to get the data directly from the sheet
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const ridersSheet = ss.getSheetByName(CONFIG.sheets.riders);
    
    if (!ridersSheet) {
      console.log('❌ Riders sheet not found, creating sample data...');
      return createSampleRidersData();
    }
    ...
    if (allData.length <= 1) {
      console.log('❌ Riders sheet is empty, creating sample data...');
      return createSampleRidersData();
    }
    ...
    console.log(`📊 Found ${dataRows.length} rows of rider data`);
    
    // Convert rows to objects
    const riders = dataRows.map((row, index) => {
      const rider = {};
      ...
      headers.forEach((header, colIndex) => {
        ...
        if (headerLower.includes('name') && !headerLower.includes('user')) {
          rider.name = rider.name || value;
        } else if (headerLower.includes('id') || headerLower.includes('number')) {
          rider.jpNumber = rider.jpNumber || value;
        ...
        // Also store with exact header name
        rider[header] = value;
      });
      
      // Ensure minimum required fields
      rider.name = rider.name || rider['Full Name'] || rider.fullName || `Rider ${index + 1}`;
      rider.jpNumber = rider.jpNumber || rider['Rider ID'] || rider.riderId || `R${String(index + 1).padStart(3, '0')}`;
      rider.status = rider.status || rider['Status'] || 'Active';
      
      return rider;
    });
    
    console.log(`✅ Converted to ${riders.length} rider objects`);
    return riders;
...
/**
 * STEP 3: Direct sheet access method
 */
function getDirectRidersData() {
  console.log('🔄 Using direct sheet access method...');
  ...
  try {
    ...
    // Look for riders sheet (case insensitive)
    let ridersSheet = null;
    for (const sheet of sheets) {
      if (sheet.getName().toLowerCase().includes('rider')) {
        ridersSheet = sheet;
        console.log(`✅ Found riders sheet: ${sheet.getName()}`);
        break;
      }
    }
    
    if (!ridersSheet) {
      console.log('❌ No riders sheet found, creating sample data...');
      return createSampleRidersData();
    }
    
    const allData = ridersSheet.getDataRange().getValues();
    
    if (allData.length <= 1) {
      console.log('❌ Sheet has no data, creating sample data...');
      return createSampleRidersData();
    }
    
    // Simple conversion - first column is name, second is ID, etc.
    const riders = [];
    for (let i = 1; i < allData.length; i++) {
      ...
      riders.push({
        name: row[0] || row[1] || `Rider ${i}`,
        jpNumber: row[1] || row[0] || `R${String(i).padStart(3, '0')}`,
        phone: row[2] || '',
        email: row[3] || '',
        status: row[4] || 'Active'
      });
    ...
    console.log(`✅ Direct method found ${riders.length} riders`);
    return riders;
...
/**
 * STEP 4: Create sample data if all else fails
 */
function createSampleRidersData() {
  console.log('🔧 Creating sample riders data...');
...
/**
 * STEP 5: Diagnostic function to test the backend
 */
function testGetPageDataForRiders() {
  ...
  try {
    ...
    console.log('  Sample rider:', result.riders && result.riders.length > 0 ? result.riders[0] : 'none');
    console.log('  Error:', result.error || 'none');
    
    if (result.success && result.riders && result.riders.length > 0) {
      console.log('🎉 SUCCESS! getPageDataForRiders is working correctly.');
    } else {
      console.log('❌ ISSUE: Function is not returning riders properly.');
    }
...
/**
 * STEP 6: Quick fix function to ensure everything works
 */
function fixRidersBackend() {
  console.log('🔧 === FIXING RIDERS BACKEND ===');
  ...
  try {
    ...
    // Test 2: Check if CONFIG is defined
    if (typeof CONFIG === 'undefined') {
      console.log('❌ CONFIG is not defined, this may cause issues');
    } else {
      console.log('✅ CONFIG is defined');
      console.log('  Riders sheet name:', CONFIG.sheets ? CONFIG.sheets.riders : 'not configured');
    }
    
    // Test 3: Test the function
    const testResult = testGetPageDataForRiders();
    
    // Test 4: Try to create riders sheet if needed
    if (!testResult.success || (testResult.riders && testResult.riders.length === 0)) {
      console.log('🔧 Attempting to create/fix riders sheet...');
      
      let ridersSheet = ss.getSheetByName('Riders');
      if (!ridersSheet) {
        ridersSheet = ss.insertSheet('Riders');
        console.log('✅ Created Riders sheet');
      }
      
      // Add headers if sheet is empty
      const dataRange = ridersSheet.getDataRange();
      if (dataRange.getLastRow() <= 1) {
        const headers = ['Rider ID', 'Full Name', 'Phone Number', 'Email', 'Status', 'Total Assignments'];
        ridersSheet.getRange(1, 1, 1, headers.length).setValues([headers]);
        
        // Add sample data
        const sampleData = [
          ['JP001', 'John Smith', '555-0101', 'john@example.com', 'Active', 15],
          ['JP002', 'Jane Doe', '555-0102', 'jane@example.com', 'Active', 12],
          ['JP003', 'Bob Wilson', '555-0103', 'bob@example.com', 'Active', 8]
        ];
        ridersSheet.getRange(2, 1, sampleData.length, headers.length).setValues(sampleData);
        console.log('✅ Added sample rider data');
      }
    }
    
    // Test 5: Final test
    const finalTest = testGetPageDataForRiders();
    
    console.log('🎯 FINAL RESULT:');
    if (finalTest.success && finalTest.riders && finalTest.riders.length > 0) {
      console.log('🎉 SUCCESS! Backend is now working correctly.');
      console.log(`📊 Found ${finalTest.riders.length} riders`);
      return { success: true, message: `Fixed! Found ${finalTest.riders.length} riders` };
    } else {
      console.log('❌ Still having issues:', finalTest.error);
      return { success: false, message: 'Fix incomplete: ' + (finalTest.error || 'Unknown issue') };
    }
...
console.log('✅ Backend riders fix loaded - run fixRidersBackend() to diagnose and fix issues');
 ...
 * =================================
 * 
 * 1. Add all the functions above to your AppServices.gs or Code.gs file
 * 2. In Google Apps Script, run: fixRidersBackend()
 * 3. This will diagnose issues and attempt to fix them automatically
 * 4. After running the fix, test your riders page again
 * 5. If you still have issues, run: testGetPageDataForRiders() to see what's returned
 */
function fixRidersSimple() {
  try {
    console.log('🔧 === SIMPLE RIDERS FIX ===');
    
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let ridersSheet = ss.getSheetByName(CONFIG.sheets.riders);
    
    // Fix 1: Create sheet if missing
    if (!ridersSheet) {
      console.log('🔧 Creating missing Riders sheet...');
      ridersSheet = ss.insertSheet(CONFIG.sheets.riders);
      
      // Add headers
      const headers = [
        'Rider ID', 'Full Name', 'Phone Number', 'Email', 'Status', 
        'Experience Level', 'Assignments Completed', 'Notes'
      ];
      ridersSheet.getRange(1, 1, 1, headers.length).setValues([headers]);
      
      // Add sample data
      const sampleData = [
        ['RIDER001', 'John Smith', '555-0101', 'john@example.com', 'Active', 'Experienced', 0, ''],
        ['RIDER002', 'Jane Doe', '555-0102', 'jane@example.com', 'Active', 'New', 0, ''],
        ['RIDER003', 'Bob Wilson', '555-0103', 'bob@example.com', 'Active', 'Experienced', 0, '']
      ];
      ridersSheet.getRange(2, 1, sampleData.length, headers.length).setValues(sampleData);
      
      console.log('✅ Created Riders sheet with sample data');
      return { success: true, message: 'Created Riders sheet with sample data' };
    }
    
    // Fix 2: Add data if sheet is empty
    const dataRange = ridersSheet.getDataRange();
    const allData = dataRange.getValues();
    
    if (allData.length <= 1) {
      console.log('🔧 Adding sample data to empty sheet...');
      ...
      const startRow = Math.max(2, allData.length + 1);
      ridersSheet.getRange(startRow, 1, sampleData.length, sampleData[0].length).setValues(sampleData);
      
      console.log('✅ Added sample rider data');
      return { success: true, message: 'Added sample rider data' };
    }
    
    // Fix 3: Clear any caches
    ...
    try {
      ...
      console.log('⚠️ Cache clearing had issues (this is usually fine)');
    }
    
    // Fix 4: Test that it works now
    console.log('🧪 Testing if fix worked...');
    try {
      const riders = getRiders();
      if (riders && riders.length > 0) {
        console.log(`✅ SUCCESS! Found ${riders.length} riders after fix`);
        return { 
          success: true, 
          message: `Fix successful - found ${riders.length} riders`,
          riders_count: riders.length
        };
      } else {
        console.log('❌ Still no riders found after fix');
        return { 
          success: false, 
          message: 'Fix applied but still no riders found - may need manual data check'
        };
      }
    } catch (error) {
      ...
      return { 
        success: false, 
        message: 'Fix applied but getRiders() still has issues: ' + error.message
      };
    }
