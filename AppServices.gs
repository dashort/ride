...
/**
 * Get detailed escort information for a specific rider within a date range
 * @param {string} riderName - The name of the rider
 * @param {Object} dateRange - Object with startDate and endDate properties
 * @return {Object} Response object with success flag and escort details
 */
function getRiderEscortDetails(riderName, dateRange) {
  try {
    debugLog(`🔍 Getting escort details for rider: ${riderName}`);
    ...
    const assignments = getRiderAssignmentsByName(riderName);
    ...
    if (dateRange && dateRange.startDate && dateRange.endDate) {
      ...
      filteredAssignments = assignments.filter(assignment => {
        try {
          ...
          debugLog(`⚠️ Error filtering assignment by date:`, error);
          return false;
        }
      });
    }
    
    // Get assignment details with hours calculation
    const assignmentsData = getAssignmentsData();
    const columnMap = assignmentsData.columnMap;
    
    const escortDetails = filteredAssignments.map(assignment => {
      let hours = 0;
      
      // Try to get actual hours from the assignment data
      const assignmentRow = assignmentsData.data.find(row => {
        const rowAssignmentId = getColumnValue(row, columnMap, CONFIG.columns.assignments.id);
        return rowAssignmentId && rowAssignmentId.toString() === assignment.assignmentId.toString();
      });
      
      if (assignmentRow) {
        // Priority 1: Use actual completion times if available
        const actualStart = getColumnValue(assignmentRow, columnMap, CONFIG.columns.assignments.actualStartTime);
        const actualEnd = getColumnValue(assignmentRow, columnMap, CONFIG.columns.assignments.actualEndTime);
        const actualDuration = getColumnValue(assignmentRow, columnMap, CONFIG.columns.assignments.actualDuration);
        
        if (actualDuration && !isNaN(parseFloat(actualDuration))) {
          // Use recorded duration if available
          hours = parseFloat(actualDuration);
          ...
          // Use estimated hours based on assignment type and times
          hours = getRealisticEscortHours(assignmentRow, columnMap);
        }
      }
    ...
    debugLog(`✅ Found ${escortDetails.length} escort details for ${riderName}`);
    ...
    console.error(`❌ Error getting escort details for rider ${riderName}:`, error);
    logError(`Error in getRiderEscortDetails for rider ${riderName}`, error);
    return { success: false, error: 'Failed to load escort details' };
  }
}
